...
namespace Script.UI.AR
{
    public class RobotBar : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            else if (_gm.judge && !_gm.observing && _gm.clientRobotBases.Any(r => r.role.Equals(new RoleT(camp, type)) && r.health > 0))
            {
                    ...
                    if (c.enabled)
                    {
                        ...
                        if (robot.GetComponentInChildren<MeshRenderer>().isVisible)
                        {
                            ...
                            switch (robot.level)
                            {
                                case 1:
                                    level.sprite = level1;
                                    level.GetComponent<RectTransform>().localScale = new Vector3(1,0.5f,1) * 0.1247058f;
                                    break;
                                case 2:
                                    level.sprite = level2;
                                    level.GetComponent<RectTransform>().localScale = new Vector3(1,0.7f,1) * 0.1247058f;
                                    break;
                                case 3:
                                    level.sprite = level3;
